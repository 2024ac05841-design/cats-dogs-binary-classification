...
def split_dataset(image_dir: str, train_ratio: float = 0.8, val_ratio: float = 0.1) -> None:
    """
...
def validate_preprocessed_images(image_dir: str, size: Tuple[int, int] = (224, 224)) -> bool:
    """
    ...
    for img_path in image_dir.rglob("*.jpg"):
        try:
            ...
            if img.size != size[::-1]:  # PIL uses (width, height)
                logger.warning(f"Image {img_path} has size {img.size}, expected {size[::-1]}")
                invalid_count += 1
        except Exception as e:
