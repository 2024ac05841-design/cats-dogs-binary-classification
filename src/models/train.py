...
def train_multiple_models(
    ...
    return results, best_model_info
            ...
            temp_save_path = f"models/temp_{model_name}.pkl"
            Path(temp_save_path).parent.mkdir(parents=True, exist_ok=True)

            # Train
            trainer = Trainer(model, device=device, model_name=model_name)
            history = trainer.train(
                train_loader, val_loader, epochs=epochs, lr=lr, save_path=temp_save_path
            )
            ...
            logger.info(f"{model_name} - Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.2f}%")
            ...
            if val_acc > best_model_info["val_acc"]:
                ...
                best_model_info["path"] = temp_save_path

        except Exception as e:
            logger.error(f"Error training {model_name}: {e}")
            results[model_name] = {"error": str(e)}

    # Save best model to separate folder
    if best_model_info["name"] is not None:
        logger.info(f"\n{'='*60}")
        logger.info(f"Best Model: {best_model_info['name']}")
        logger.info(
            f"Validation Accuracy: {best_model_info['val_acc']:.2f}%"
        )
        logger.info(f"Validation Loss: {best_model_info['val_loss']:.4f}")
        logger.info(f"{'='*60}\n")

        # Create best model directory
        best_model_path = Path(best_model_dir) / f"best_model_{best_model_info['name']}.pkl"
        ...
        for model_name, result in results.items():
            if "path" in result and Path(result["path"]).exists():
                ...
                logger.info(f"Cleaned up temporary file: {result['path']}")
